...
public:
    ...
    void Draw(const glm::mat4& view, const glm::mat4& projection) const override {
        ...
        // TODO: Draw I/O socket indicators
    }
        ...
        for(auto& [x, y] : sswitch.switchSocketOffsets) {
            ...
            switch(m_direction) {
                case Direction::NORTH:
                    m_sockets.m_sockets.push_back(Socket(glm::ivec2(m_gridPosition.x + y, m_gridPosition.y - x), GlobalGrid::GetGrid()->GetGridPointPositionRelative(m_position, x, y)));
                    break;
                case Direction::SOUTH:
                    m_sockets.m_sockets.push_back(Socket(glm::ivec2(m_gridPosition.x - y, m_gridPosition.y + x), GlobalGrid::GetGrid()->GetGridPointPositionRelative(m_position, x, y)));
                    break;
                case Direction::EAST:
                    m_sockets.m_sockets.push_back(Socket(glm::ivec2(m_gridPosition.x + x, m_gridPosition.y + y), GlobalGrid::GetGrid()->GetGridPointPositionRelative(m_position, y, x)));
                    break;
                case Direction::WEST:
                    m_sockets.m_sockets.push_back(Socket(glm::ivec2(m_gridPosition.x - x, m_gridPosition.y - y), GlobalGrid::GetGrid()->GetGridPointPositionRelative(m_position, y, x)));
                    break;
